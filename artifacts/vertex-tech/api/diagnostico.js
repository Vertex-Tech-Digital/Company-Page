...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    try {
      ...
      console.error("[Diagnostico PDF Error]:", pdfErr.message);
    }
    ...
    if (
      ...
      try {
        ...
        // En desarrollo local resolvemos rápido usando DNS públicos
        if (process.env.NODE_ENV !== "production") {
          try {
        ...
        const transporter = nodemailer.createTransport({
          ...
          port: 465,
          secure: true,
          connectionTimeout: 3000, // Ajustado a tiempos óptimos para Serverless
        ...
        // Promise.race garantiza que la función no muera por timeout de Vercel (máx 5s de espera)
        await Promise.race([
          Promise.all([clientMail, adminMail]),
        ...
        console.error("[Diagnostico Email Error]:", emailErr.message);
      }
    } else {
      console.warn(
